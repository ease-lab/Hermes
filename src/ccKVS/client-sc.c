...
///								(GRH_SIZE + sizeof(struct wrkr_coalesce_mica_op)) : UD_REQ_SIZE ;
    ...
    struct hrd_ctrl_blk *cb = hrd_ctrl_blk_init(clt_gid,	/* local_hid */
            ...
            ///CLIENT_UD_QPS, remote_buf_size + SC_CLT_BUF_SIZE,	/* num_dgram_qps, dgram_buf_size */
                                                CLIENT_UD_QPS, SC_CLT_BUF_SIZE,	/* num_dgram_qps, dgram_buf_size */
            ///MASTER_SHM_KEY + WORKERS_PER_MACHINE + local_client_id, /* key */
                                                MASTER_SHM_KEY + local_client_id, /* key */
    ...
    struct ud_req *incoming_reqs = (struct ud_req *) cb->dgram_buf;
    /* ---------------------------------------------------------------------------
    ------------------------------MULTICAST SET UP-------------------------------
    ---------------------------------------------------------------------------*/
    ...
    ---------------------------------------------------------*/
    setup_client_conenctions_and_spawn_stats_thread(clt_gid, cb); ///WARNING: not checked
    ...
    struct ibv_sge coh_recv_sgl[MAX_COH_RECEIVES], credit_recv_sgl;
    ...
    uint8_t credits_for_invs[MACHINE_NUM], credits_for_acks[MACHINE_NUM], credits_for_upds[MACHINE_NUM],
            credits[VIRTUAL_CHANNELS][MACHINE_NUM];
    ...
    set_up_coh_ops(&update_ops, &ack_bcast_ops, &inv_ops, &inv_to_send_ops,
                   update_resp, inv_resp, &coh_buf, protocol);
...
//	}
    ...
    while(1) {
        ...
        if (WRITE_RATIO > 0 && DISABLE_CACHE == 0) {
            ...
            credit_wr_i = forge_credits_SC(coh_message_count, acks_seen, invs_seen, upds_seen, local_client_id,
...
//		rem_req_i = 0;
//		empty_req_percentage = w_stats[local_client_id].empty_reqs_per_trace;// / rolling_iter;
...
//									&latency_info, &start, &local_measure, hottest_keys_pointers);
        ...
        ------------------------------POLL & SEND----------------------------------
        ---------------------------------------------------------------------------*/
//		w_stats[local_client_id].remotes_per_worker += rem_req_i;
