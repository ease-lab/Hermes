...
// Change accordingly
#define BV_BIT_VECTOR_SIZE 8 // Set if you use statical bit vector (bit_vector_t)
...
bv_bit_get_internal(const uint8_t* bit_array, uint16_t size_in_bits, uint8_t bit)
{
...
static inline uint8_t
bv_are_equal_internal(uint8_t* ba1, uint16_t size_in_bits1,
                      uint8_t* ba2, uint16_t size_in_bits2)
{
    ...
    uint8_t unused_ms_bits = (uint8_t) (BV_BITS_IN_A_BYTE * size_in_bytes - size_in_bits1);
    ...
    return (uint8_t) (memcmp(ba1, ba2, (size_t) (size_in_bytes - 1)) == 0 &&
                      last_byte1 == last_byte2 ? 1 : 0);
}

static inline void
bv_copy_internal(uint8_t* ba_dst, uint16_t size_in_bits_dst,
                 uint8_t* ba_src, uint16_t size_in_bits_src)
{
...
{
    return bv_are_equal_internal(bv1.bit_array, bv1.bv_size, bv2.bit_array, bv2.bv_size);
}
...
{
    bv_and_internal(bv_dst->bit_array, bv_dst->bv_size, bv_src.bit_array, bv_src.bv_size);
}

static inline void
dbv_or(dbit_vector_t *bv_dst, dbit_vector_t bv_src)
{
    bv_or_internal(bv_dst->bit_array, bv_dst->bv_size, bv_src.bit_array, bv_src.bv_size);
}
...
{
    return bv_are_equal_internal(bv1.bit_array, BV_BIT_VECTOR_SIZE, bv2.bit_array, BV_BIT_VECTOR_SIZE);
}

static inline void
bv_copy(bit_vector_t *bv_dst, bit_vector_t bv_src)
{
    bv_copy_internal(bv_dst->bit_array, BV_BIT_VECTOR_SIZE, bv_src.bit_array, BV_BIT_VECTOR_SIZE);
}
...
{
    bv_and_internal(bv_dst->bit_array, BV_BIT_VECTOR_SIZE, bv_src.bit_array, BV_BIT_VECTOR_SIZE);
}

static inline void
bv_or(bit_vector_t *bv_dst, bit_vector_t bv_src)
{
    bv_or_internal(bv_dst->bit_array, BV_BIT_VECTOR_SIZE, bv_src.bit_array, BV_BIT_VECTOR_SIZE);
}
