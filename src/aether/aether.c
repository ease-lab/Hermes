...
---------------------------------------------------------------------------*/
static inline void
_internal_aether_post_recvs(struct hrd_ctrl_blk *cb, ud_channel_t *ud_c, uint16_t num_of_receives);
static inline void
_internal_aether_post_crd_recvs(struct hrd_ctrl_blk *cb, ud_channel_t *ud_c, uint16_t num_recvs);
...
aether_setup_incoming_buff_and_post_initial_recvs(ud_channel_t* ud_c, struct hrd_ctrl_blk *cb)
{
	...
	if(ud_c->type != CRD)
		_aether_post_recvs(cb, ud_c, ud_c->max_recv_wrs);
	else
		_aether_post_crd_recvs(cb, ud_c, ud_c->max_recv_wrs);
}
//
///* --------------------------------------------------------------------------
...
//
//    uint16_t max_req_size = _aether_ud_recv_max_pkt_size(ud_channel);
//
//	//poll completion q
...
//		_aether_inc_recv_pull_ptr(ud_channel);
//
//
//		if(AETHER_ENABLE_ASSERTIONS)
...
//	}
//
//	_aether_inc_send_push_ptr(ud_channel); //go to the next pkt
//}
//
//uint8_t
