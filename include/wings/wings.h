...
---------------------------------------------------------------------------*/
static inline void
_wings_post_crd_recvs(ud_channel_t *ud_c, uint16_t num_recvs)
{
    if(WINGS_ENABLE_ASSERTIONS)
		assert(ud_c->type == CRD);
...
_wings_poll_crds_and_post_recvs(ud_channel_t *ud_c)
{
	...
	if(crd_pkts_found > 0) {
		...
		_wings_post_crd_recvs(ud_c, (uint16_t) crd_pkts_found);
...
// dst_qp_id is ignored if its a bcast channel
{
	...
	uint8_t* next_req_ptr = _wings_get_n_msg_ptr_from_send_pkt(ud_c, curr_pkt_ptr, curr_pkt_ptr->req_num);
				 ...
				 copy_and_modify_input_elem_t copy_and_modify_elem)
{
	...
	if(WINGS_ENABLE_ASSERTIONS)
		assert(_wings_curr_send_pkt_ptr(ud_c)->req_num == 0);
	...
	for (int i = 0; i < input_array_len; i++) {
		...
		if(_wings_curr_send_pkt_ptr(ud_c)->req_num == ud_c->max_coalescing)
			_wings_check_if_batch_n_inc_pkt_ptr(ud_c, &pkts_in_batch, &msgs_in_batch);
	}
