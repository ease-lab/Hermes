...
typedef struct _ud_channel_t
{
    ...
    wings_ud_send_pkt_t* send_pkt_buff; /// Intermediate buffs where reqs are copied when pkts are send
...
void wings_setup_channel_qps_and_recvs(ud_channel_t **ud_c_array, uint16_t ud_c_num,
                                       dbit_vector_t *shared_rdy_var, uint16_t worker_lid);

/// Main functions
static inline uint16_t
wings_poll_buff_and_post_recvs(ud_channel_t *ud_c, uint16_t max_pkts_to_poll,
