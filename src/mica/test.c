...
void *test_batch_op(void *arg)
{
	...
	while(1) {
		...
		for(i = 0; i < TEST_NUM_KEYS; i++) {
			unsigned long long *op_key =
				(unsigned long long *) &op_arr[i].key;
			op_key[0] = key_arr[i].first;
			op_key[1] = key_arr[i].second;

			op_arr[i].val_len = TEST_VAL_LEN;
			memset(op_arr[i].value,
				(uint8_t) op_arr[i].key.tag, op_arr[i].val_len);
		}
		...
		printf("Thread %d: Throughput = %.2f M/s, fail fraction = %.4f. "
			"sum = %llu, seed = %llu\n",
			tid, TEST_NUM_KEYS / (seconds * 1000000),
			(double) kv.num_get_fail / kv.num_get_op, sum,
